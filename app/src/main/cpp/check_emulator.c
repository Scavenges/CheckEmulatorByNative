...
// 这里是模仿java代码的检查是否为模拟器 必要参数 context
    ...
    if(startsWith(fingerprintchars,"generic") ){//是模拟器 不通过
        return 0;
    }
    if(startsWith(fingerprintchars,"unknown") ){//是模拟器 不通过
        return 0;
    }
    ...
    if(startsWith(brandchars,"generic") && startsWith(devicechars,"generic")){
        return 0;
    }

    if(equals(productchars,"google_sdk")){
        return 0;
    }
